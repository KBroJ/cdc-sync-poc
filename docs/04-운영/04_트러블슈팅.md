...
| 2026-01-14 | TOBE→ASIS 동기화 | **ASIS 프로시저/함수 전체 누락** | 13 |
| 2026-01-14 | TOBE→ASIS 동기화 | **SYNC_CODE_MAPPING 매핑 데이터 누락** | 13.2 |
| 2026-01-14 | DELETE 처리 | **TARGET_NOT_FOUND 처리 로직 추가** | 13.3 |
...
## 13. ASIS 프로시저/함수 누락 문제 (2026-01-14)

### 13.1 ASIS 프로시저/함수 전체 누락

> **발생 시점**: 2026-01-14 / TOBE에서 DELETE 후 동기화 흐름 확인 시
> **상황**: TOBE에서 DELETE 실행 시 "성공"으로 표시되나, STAGING 테이블이 비어있음

#### 증상
- TOBE에서 DELETE 실행 → 시뮬레이터에서 "성공" 표시
- 동기화 흐름 탭에서 STAGING_ASIS_BOOK이 비어있음
- ASIS 원본 테이블에 변경 없음
...
3. ASIS Scheduler Job 확인 → JOB_CDC_WORKER 없음!
   └─ TOBE에는 있는데 ASIS에는 없음
...
```sql
-- ASIS Oracle에 접속
docker exec -i asis-oracle sqlplus ASIS_USER/asis123@//localhost:1521/XEPDB1
...
-- Job 확인
SELECT job_name, state FROM USER_SCHEDULER_JOBS WHERE job_name = 'JOB_CDC_WORKER';
```

> **핵심**: Docker 볼륨이 남아있으면 init 스크립트가 재실행되지 않습니다. `docker-compose down -v` 후 재시작하거나 수동으로 오브젝트를 생성해야 합니다.

---

### 13.2 SYNC_CODE_MAPPING 매핑 데이터 누락

> **발생 시점**: 2026-01-14 / 프로시저 생성 후 CDC 처리 시
> **상황**: CDC 테이블의 데이터가 'E'(Error) 상태로 처리됨

#### 증상
```sql
SELECT PROCESSED_YN, COUNT(*) FROM CDC_ASIS_BOOK GROUP BY PROCESSED_YN;
-- E    5  (에러 상태)
```

#### 에러 메시지
```
ORA-12899: value too large for column "ASIS_USER"."STAGING_ASIS_BOOK"."CATEGORY"
(actual: 3, maximum: 2)
```

#### 원인
SYNC_CODE_MAPPING 테이블에 매핑 데이터가 없어서 `FN_CONVERT_CODE` 함수가 원본값을 그대로 반환

```
TOBE 코드값: LIT (3자리)
ASIS 컬럼: CATEGORY VARCHAR2(2)
매핑이 없으면: LIT → LIT (3자리를 2자리 컬럼에 넣으려다 실패)
```
...
-- 에러 데이터 재처리
UPDATE CDC_ASIS_BOOK SET PROCESSED_YN = 'N', ERROR_MSG = NULL WHERE PROCESSED_YN = 'E';
COMMIT;
```

> **핵심**: 03_create_mapping_tables.sql 스크립트도 실행되지 않았을 가능성이 있습니다. 매핑 테이블은 있지만 데이터가 없는지 확인하세요.

---

### 13.3 TARGET_NOT_FOUND 처리 추가

> **발생 시점**: 2026-01-14 / DELETE 동기화 동작 확인 시
> **상황**: TOBE에만 있고 ASIS에 없는 데이터를 DELETE하면 어떻게 되는가?

#### 기존 동작
```sql
DELETE FROM BOOK_INFO WHERE BOOK_ID = 9999;
-- 결과: 0 rows deleted (에러 아님)
-- CDC_SYNC_LOG: SUCCESS (실제로는 삭제된 것이 없음)
```

#### 문제점
대상이 없는 DELETE도 SUCCESS로 처리되어 추적이 어려움

#### 개선 내용
SP_WORKER_BOOK 프로시저에 `SQL%ROWCOUNT` 확인 로직 추가:
...
#### 확인 결과
```sql
SELECT STATUS, ERROR_MSG FROM CDC_SYNC_LOG WHERE PK_VALUE = '8888';
-- TARGET_NOT_FOUND    삭제할 대상이 존재하지 않음 (이미 삭제됨)
```

#### 적용 범위
- ASIS: SP_WORKER_BOOK (DELETE, UPDATE)
- TOBE: SP_WORKER_BOOK (DELETE, UPDATE)

> **핵심**: `SQL%ROWCOUNT`를 사용하면 실제 영향받은 행 수를 확인할 수 있습니다. 0이면 대상이 없었음을 의미합니다.
