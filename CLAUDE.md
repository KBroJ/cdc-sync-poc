...
| 목적 | 문서 | 내용 |
|------|------|------|
| **처음 시작** | `docs/00-시작하기/01_프로젝트_소개.md` | 프로젝트 배경, 왜 CDC가 필요한지 |
| **환경 실행** | `docs/00-시작하기/02_환경_시작_가이드.md` | Docker 환경 시작 방법 |
| **전체 구현** | `docs/03-구현/01_구현_가이드.md` | **통합 구현 가이드 (개념+이유+코드)** |
| **아키텍처** | `docs/01-개념/02_아키텍처_개요.md` | 전체 아키텍처 도식 |
| **설계 상세** | `docs/02-설계/01_동기화_설계.md` | 전체 설계, 스키마 매핑 정의 |
| **문제 해결** | `docs/04-운영/04_트러블슈팅.md` | 자주 발생하는 문제 해결 |
...
| 유형 | 위치 | 파일명 패턴 |
|------|------|-------------|
| 트러블슈팅 | `docs/04-운영/04_트러블슈팅.md` | 기존 파일에 추가 |
| 개발 기록 | `docs/03-구현/02_개발_기록.md` | 기존 파일에 추가 |
| 설계 변경 | `docs/02-설계/` | 해당 설계 문서에 추가 |

### 트러블슈팅 문서

- **통합 가이드**: `docs/04-운영/04_트러블슈팅.md` (인프라+코드+스키마 문제 모두 포함)
